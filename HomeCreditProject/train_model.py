...
def sanitize_feature_names(dataframe):
    """Make column names safe for modern LightGBM and keep them unique."""
...
def kfold_lightgbm_sklearn(data, categorical_feature=None):
    """Train LightGBM with guaranteed-safe internal feature names."""

    data = sanitize_feature_names(data)
    df = data[data['TARGET'].notnull()].copy()
    test = data[data['TARGET'].isnull()].copy()

    print("Train/valid shape: {}, test shape: {}".format(df.shape, test.shape))

    del_features = ['TARGET', 'SK_ID_CURR', 'SK_ID_BUREAU', 'SK_ID_PREV', 'index', 'level_0']
    original_predictors = [c for c in df.columns if c not in del_features]

    # Use simple generated names internally. This completely avoids LightGBM
    # JSON/special-character restrictions, regardless of original column names.
    safe_predictors = [f'feature_{i}' for i in range(len(original_predictors))]

    X = df[original_predictors].copy()
    X_test = test[original_predictors].copy()
    X.columns = safe_predictors
    X_test.columns = safe_predictors

    # LightGBM can handle numeric and category columns. Convert any remaining
    # object columns to categorical codes consistently.
    for col in safe_predictors:
        if X[col].dtype == 'object' or str(X[col].dtype) == 'category':
            combined = pd.concat([X[col], X_test[col]], axis=0).astype('category')
            X[col] = combined.iloc[:len(X)].cat.codes.astype('int32')
            X_test[col] = combined.iloc[len(X):].cat.codes.astype('int32').values
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(X, df['TARGET'])):
        print('\n========== FOLD {} =========='.format(n_fold + 1))

        train_x = X.iloc[train_idx]
        train_y = df['TARGET'].iloc[train_idx]
        valid_x = X.iloc[valid_idx]
        valid_y = df['TARGET'].iloc[valid_idx]

        params = {'random_state': RANDOM_SEED, 'n_jobs': NUM_THREADS}
        ...
        best_iteration = clf.best_iteration_ if clf.best_iteration_ else None
        oof_preds[valid_idx] = clf.predict_proba(valid_x, num_iteration=best_iteration)[:, 1]
        sub_preds += clf.predict_proba(X_test, num_iteration=best_iteration)[:, 1] / folds.n_splits

        fold_importance = pd.DataFrame({
            'feature': original_predictors,
            'gain': clf.booster_.feature_importance(importance_type='gain'),
            'split': clf.booster_.feature_importance(importance_type='split')
        })
        importance_df = pd.concat([importance_df, fold_importance], axis=0, ignore_index=True)

        print('Fold %2d AUC : %.6f' % (n_fold + 1, roc_auc_score(valid_y, oof_preds[valid_idx])))

        model_path = os.path.join('models', 'lightgbm_fold_{}.txt'.format(n_fold + 1))
    ...
    if GENERATE_SUBMISSION_FILES:
        ...
        oof.to_csv(os.path.join('outputs', 'oof_predictions.csv'), index=False)

        submission = pd.DataFrame({
            'SK_ID_CURR': test['SK_ID_CURR'].values,
            'TARGET': sub_preds
        })
        submission.to_csv(os.path.join('outputs', 'submission.csv'), index=False)

        mean_importance.to_csv(os.path.join('outputs', 'feature_importance.csv'), index=False)
