...
import { DEMO_APPLICANT, EMPTY_APPLICANT, } from "../types/assessment";
import { calculateCreditScore } from "../services/scoringEngine";
import { fetchApplicantById, submitAssessmentRecord, runAndSavePrediction, } from "../services/supabaseService";
// ─── Helpers ─────────────────────────────────────────────────────────────────
...
export const AssessmentProvider = ({ children, }) => {
    ...
    // Applicant records
    const [originalApplicantData, setOriginalApplicantData] = useState(null);
    const [formData, setFormData] = useState(() => {
        try {
            const saved = localStorage.getItem("credinova_form_data");
            return saved ? JSON.parse(saved) : EMPTY_APPLICANT;
        }
        catch {
    ...
    const loadApplicantById = async (id) => {
            ...
            if (!isNaN(applicantId) && applicantId > 0) {
                ...
                if (pred.shap_summary && pred.shap_summary.length > 0) {
                    ...
                    if (positiveDrivers.length > 0)
                        topPositives = positiveDrivers.slice(0, 4);
                    if (riskDrivers.length > 0)
                        topRisks = riskDrivers.slice(0, 4);
                }
                const mappedResult = {
                    ...baseScore,
                    ...
                    riskLevel: pred.risk_band === "Good" || pred.risk_band === "Excellent"
                        ? "LOW RISK"
                        : pred.risk_band === "Fair" || pred.risk_band === "Moderate"
                            ? "MEDIUM RISK"
                            : "HIGH RISK",
                    recommendation: pred.credit_score >= 720
                        ? "APPROVE"
    ...
    const handleSetLookupId = (id) => {
        setLookupId(id);
    };
    return (_jsx(AssessmentContext.Provider, { value: {
            currentStep,
            setCurrentStep,
            handleNext,
            handleBack,
