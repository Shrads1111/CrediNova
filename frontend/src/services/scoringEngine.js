...
export function calculateCreditScore(data) {
    ...
    if (annuityToIncome < 0.10)
        score += 60;
    else if (annuityToIncome < 0.20)
        score += 30;
    else if (annuityToIncome < 0.30)
        score += 10;
    else if (annuityToIncome < 0.40)
        score -= 20;
    else if (annuityToIncome < 0.50)
        score -= 50;
    else
        score -= 90;
    ...
    if (creditToIncome < 2)
        score += 40;
    else if (creditToIncome < 4)
        score += 15;
    else if (creditToIncome < 6)
        score -= 15;
    else if (creditToIncome < 10)
        score -= 40;
    else
        score -= 70;
    ...
    if (regionRating === 1)
        score += 25;
    else if (regionRating === 3)
        score -= 30;
    // 7. Age signal (very young or very old = slightly higher risk)
    if (ageYears >= 35 && ageYears <= 55)
        score += 20;
    else if (ageYears >= 25 && ageYears < 35)
        score += 10;
    else if (ageYears < 22 || ageYears > 65)
        score -= 20;
    // 8. Employment stability
    if (employedYears >= 5)
        score += 30;
    else if (employedYears >= 2)
        score += 15;
    else if (employedYears >= 1)
        score += 5;
    else if (employedYears <= 0)
        score -= 25;
    // 9. Income type bonus
    const incomeType = (data.NAME_INCOME_TYPE ?? "").toLowerCase();
    if (incomeType.includes("state"))
        score += 20;
    else if (incomeType.includes("commercial"))
        score += 15;
    else if (incomeType.includes("working"))
        score += 10;
    else if (incomeType.includes("pension"))
        score += 5;
    else if (incomeType.includes("unemployed"))
        score -= 60;
    // 10. Education bonus
    const edu = (data.NAME_EDUCATION_TYPE ?? "").toLowerCase();
    if (edu.includes("academic"))
        score += 25;
    else if (edu.includes("higher"))
        score += 15;
    else if (edu.includes("incomplete higher"))
        score += 5;
    else if (edu.includes("lower"))
        score -= 20;
    // Clamp to 300–960
    ...
    // ── Default probability ────────────────────────────────────────────────────
    const defaultProbNum = clamp(parseFloat(((1000 - creditScore) / 1000 * 14).toFixed(1)), 0.5, 22);
    ...
    const positives = [];
    if (extMean >= 0.6) {
        positives.push(`Strong external risk composite (${extMean.toFixed(3)}) signals low historical default likelihood.`);
    }
    else if (extMean >= 0.4) {
        positives.push(`Moderate external risk composite (${extMean.toFixed(3)}) within acceptable risk threshold.`);
    }
    if (annuityToIncome > 0 && annuityToIncome < 0.25) {
        positives.push(`Healthy annuity-to-income ratio of ${(annuityToIncome * 100).toFixed(1)}% — well within prudential ceiling.`);
    }
    if (employedYears >= 3) {
        positives.push(`${employedYears.toFixed(1)} years of continuous employment demonstrates income stability.`);
    }
    if (def30 === 0 && def60 === 0) {
    ...
    if (annuityToIncome >= 0.35) {
        risks.push(`Elevated annuity burden (${(annuityToIncome * 100).toFixed(1)}% of income) increases repayment stress probability.`);
    }
    if (def30 > 0 || def60 > 0) {
        risks.push(`${def30 + def60} default event${def30 + def60 > 1 ? "s" : ""} recorded in applicant's social circle.`);
    }
    if (bureauQtr > 3) {
    ...
    const aiExplanation = `${applicantLabel}'s composite credit score of ${creditScore} out of 1000 reflects a ` +
