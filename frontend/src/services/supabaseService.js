...
import { CONTRACT_TYPES, EDUCATION_TYPES, FAMILY_STATUSES, HOUSING_TYPES, INCOME_TYPES, OCCUPATION_TYPES, ORGANIZATION_TYPES, } from "../types/assessment";
import { API_BASE, createAssessment as apiCreateAssessment, predictApplicant as apiPredictApplicant, getApplicant as apiGetApplicant, listApplicants as apiListApplicants, } from "./api";
// ─── Helpers to map raw tabular values into form options ────────────────────
function mapContractType(val) {
    if (typeof val === "string" && CONTRACT_TYPES.includes(val))
        return val;
    const num = Number(val);
    return num === 1 ? "Revolving loans" : "Cash loans";
}
function mapGender(val) {
    if (typeof val === "string" && (val === "M" || val === "F"))
        return val;
    const num = Number(val);
    return num === 1 ? "M" : "F";
}
function mapCar(val) {
    if (typeof val === "string" && (val === "Y" || val === "N"))
        return val;
    const num = Number(val);
    return num === 1 ? "Y" : "N";
}
function mapDays(val) {
...
function mapIndexed(val, list) {
    if (typeof val === "string" && list.includes(val))
        return val;
 ...
 */
export function rowToApplicantFormData(row) {
 ...
 */
export async function fetchApplicantById(applicantId) {
 ...
 */
export async function fetchSampleApplicants(limit = 10) {
    try {
        const { data, error } = await supabase
            .from("demo_applicants")
 ...
 */
export async function submitAssessmentRecord(formData, customerId) {
 ...
 */
export async function fetchRecentAssessments(limit = 10) {
    try {
        const { data, error } = await supabase
            .from("assessments")
            .select("*")
